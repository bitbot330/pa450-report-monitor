...
2. 每次分析開始前，runtime 必須掃描 active feedback folder 裡的 `report_YYYYMMDD.md`：從 Review UI 觸發重新分析時使用 UI 目前選的回報資料夾； standalone `src/analyze.py` 則維持掃描 `output/`。只處理日期晚於 `.agent/review_state.json` checkpoint 的 feedback 檔。
...
7. AI 不得假裝自己讀取或寫入 `.agent/review.md`、`.agent/review_state.json` 或 active feedback folder 裡的 `report_YYYYMMDD.md`。
